...
public class  ScoreAdapter extends BaseListAdapter {
    ...
    public ScoreAdapter(List<StudentScoreBean> data, OnItemClickListener listener) {
        this.mData = data;
        this.mListener = listener;
    }

    public interface OnItemClickListener {
        void onItemClick(View view, ArrayList<StudentScoreBean.ScorelistBean> scorelist);
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, final int position) {
        if (mData != null && mData.size() > 0 && holder instanceof ItemViewHolder) {
            ...
            itemViewHolder.classNumTv.setText(mData.get(position).getScorelist().get(position).getClass_number());

            itemViewHolder.lookDetailTv.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    if (mListener != null) {
                        mListener.onItemClick(view, (mData.get(position).getScorelist()));
                    }
                }
            });
